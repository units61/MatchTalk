...
class MatchingService {
    constructor() {
        this.io = null;
        this.QUEUE_MALE = 'queue:male';
        this.QUEUE_FEMALE = 'queue:female';
    }
    /**
    ...
    async joinQueue(userId) {
        ...
        const otherQueueKey = user.gender === 'male' ? this.QUEUE_FEMALE : this.QUEUE_MALE; // For check check
        ...
        const pos = await redis_1.redis.lpos(queueKey, userId);
        if (pos !== null) {
            throw new errors_1.HttpError(400, 'Zaten eşleştirme kuyruğundasınız');
        }
        // Aktif oda kontrolü (DB'den)
        const activeRoom = await prisma_1.prisma.roomParticipant.findFirst({
            where: {
                userId,
                room: {
                    timeLeftSec: { gt: 0 },
        ...
        if (activeRoom) {
            throw new errors_1.HttpError(400, 'Zaten bir odadasınız');
        }
        // Kuyruğa ekle
        await redis_1.redis.rpush(queueKey, userId);
        ...
        return {
            ...
            status: 'WAITING',
            position: await redis_1.redis.llen(queueKey),
            createdAt: new Date(),
        };
    }
    /**
    ...
    async leaveQueue(userId) {
        ...
        const removedMale = await redis_1.redis.lrem(this.QUEUE_MALE, 0, userId);
        const removedFemale = await redis_1.redis.lrem(this.QUEUE_FEMALE, 0, userId);
    ...
    async getQueueStatus(userId) {
        ...
        let pos = await redis_1.redis.lpos(this.QUEUE_MALE, userId);
        let queueKey = this.QUEUE_MALE;
        if (pos === null) {
            pos = await redis_1.redis.lpos(this.QUEUE_FEMALE, userId);
        ...
        const totalWaiting = await redis_1.redis.llen(queueKey);
        return {
            inQueue: true,
     ...
     */
    async checkAndMatch() {
        const maleCount = await redis_1.redis.llen(this.QUEUE_MALE);
        const femaleCount = await redis_1.redis.llen(this.QUEUE_FEMALE);
        ...
        const males = await redis_1.redis.lpop(this.QUEUE_MALE, 4);
        if (!males || males.length < 4) {
            // Yetersiz sayı, geri koy
            if (males && males.length > 0) {
                await redis_1.redis.lpush(this.QUEUE_MALE, ...males);
            }
            return;
        }
        const females = await redis_1.redis.lpop(this.QUEUE_FEMALE, 4);
        if (!females || females.length < 4) {
            // Yetersiz sayı, erkekleri de geri koy
            if (females && females.length > 0) {
                await redis_1.redis.lpush(this.QUEUE_FEMALE, ...females);
            }
        ...
        try {
            // Odayı oluştur
            const room = await roomsService_1.roomsService.createRoom(selectedUserIds[0], {
            ...
            for (let i = 1; i < selectedUserIds.length; i++) {
                try {
                    await roomsService_1.roomsService.joinRoom(selectedUserIds[i], {
                        roomId: room.id,
                    });
                }
                catch (e) {
            ...
            if (this.io) {
                ...
                for (const user of users) {
                    (0, server_1.emitToUser)(this.io, user.id, 'match-found', {
                        roomId: room.id,
                        room: roomData,
                        matchedUsers: users,
                    });
    ...
    // Admin/Debug için
    async getQueueUsers() {
        const males = await redis_1.redis.lrange(this.QUEUE_MALE, 0, 10);
        const females = await redis_1.redis.lrange(this.QUEUE_FEMALE, 0, 10);
