...
class RoomsService {
    ...
    async createRoom(userId, input) {
        ...
        const existingParticipation = await prisma_1.prisma.roomParticipant.findFirst({
            where: {
                userId,
                room: {
                    timeLeftSec: {
    ...
    async joinRoom(userId, input) {
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (room.timeLeftSec === 0 && room.participants.length === room.maxParticipants) {
                throw new errors_1.HttpError(400, 'Oda süresi dolmuş');
            }
            if (room.participants.length >= room.maxParticipants) {
                throw new errors_1.HttpError(400, 'Oda dolu');
            }
            const existingParticipation = room.participants.find(p => p.userId === userId);
            if (existingParticipation) {
                throw new errors_1.HttpError(400, 'Zaten bu odadasınız');
            }
            // Başka oda kontrolü
            ...
            if (otherRoomParticipation) {
                throw new errors_1.HttpError(400, 'Başka bir odadasınız');
            }
            // 3. Odaya katıl
        ...
        if (!result)
            throw new errors_1.HttpError(500, 'İşlem başarısız');
        ...
        if (shouldStartTimer) {
            ...
            timerService_1.timerService.startTimer(input.roomId).catch(err => console.error('Timer start error:', err));
        }
        return {
            id: result.id,
            name: result.name,
    ...
    async leaveRoom(userId, roomId) {
        // Katılım kontrolü
        const participation = await prisma_1.prisma.roomParticipant.findUnique({
            where: {
                userId_roomId: {
                    userId,
                    roomId,
        ...
        if (!participation) {
            throw new errors_1.HttpError(404, 'Bu odada değilsiniz');
        }
        // Odadan ayrıl
        ...
        const remainingParticipants = await prisma_1.prisma.roomParticipant.count({
            where: { roomId },
        });
